...
open("res_JR_fixed_U/norm_squared_U_025_delta_$arg1.txt", "w") do file
    for i in 1:N
...
open("res_JR_fixed_U/flc_theta_U_025_delta_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)
        println(file, "$(System_sizes[i]),$(fluct_th[i])")
    end
end

open("res_JR_fixed_U/flc_phi_U_025_delta_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)
        println(file, "$(System_sizes[i]),$(fluct_ph[i])")
    end
end

open("res_JR_fixed_U/prob_dens_U_025_delta_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)

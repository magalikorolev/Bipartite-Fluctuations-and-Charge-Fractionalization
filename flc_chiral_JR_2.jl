...
open("res_JR_400/norm_squared_delta_05_U_$arg1.txt", "w") do file
    for i in 1:N
...
open("res_JR_400/flc_theta_delta_05_U_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)
        println(file, "$(System_sizes[i]),$(fluct_th[i])")
    end
end

open("res_JR_400/flc_phi_delta_05_U_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)
        println(file, "$(System_sizes[i]),$(fluct_ph[i])")
    end
end

open("res_JR_400/prob_dens_delta_05_U_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)

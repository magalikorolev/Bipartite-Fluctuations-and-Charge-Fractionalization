...
open("res_JR_bis/norm_squared_delta_$arg2"*"_U_$arg1.txt", "w") do file
...
open("res_JR_bis/flc_theta_delta_$arg2"*"_U_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)
        println(file, "$(System_sizes[i]),$(fluct_th[i])")
    end
end

open("res_JR_bis/flc_phi_delta_$arg2"*"_U_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)
        println(file, "$(System_sizes[i]),$(fluct_ph[i])")
    end
end

open("res_JR_bis/prob_dens_delta_$arg2"*"_U_$arg1.txt", "w") do file
    for i in 1:length(System_sizes)
